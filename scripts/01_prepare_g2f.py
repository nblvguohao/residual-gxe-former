...
def parse_args():
    parser = argparse.ArgumentParser(description='Prepare G2F data into unified schema.')
...
def main():
    ...
    raw_dir = args.raw_dir or Path(data_config.get("raw_dir", "data/raw/g2f"))
    target_trait = data_config.get("target_trait", "grain_yield")
    audit_path = ROOT / "outputs" / "audit" / "g2f_数据审计报告.md"
    result = prepare_g2f(
        raw_dir=raw_dir,
    ...
    if not result.ok:
        ...
        raise SystemExit(f"G2F raw data is incomplete; missing categories: {missing}")
